...
description: Model releases, product updates, and industry moves, each with a one-line take. Hand-curated by sam.
editLink: false
lastUpdated: false
isNoComment: true
isNoBackBtn: true
...
<template v-for="[month, group] in monthGroups" :key="month">
  <h2 :id="month" class="news-month">{{ monthLabel(month, 'en') }}</h2>
  <article v-for="item in group" :key="item.url" class="news-row">
    <time>{{ item.date }}</time>
    <div>
      <a :href="item.url" target="_blank" rel="noopener noreferrer">{{ item.title.en }} <span aria-hidden="true">↗</span></a>
      <span class="news-meta">{{ kindLabel(item.kind, 'en') }} · {{ item.source }}</span>
      <p>{{ item.comment.en }}</p>
    </div>
  </article>
</template>
